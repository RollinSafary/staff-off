...
class MockApi implements ApiStructure {
  ...
  // User section
  user = {
    getProfile: createMockEndpoint<null, UserProfile>('user-getProfile', async () => {
      return {
        id: 'mock-user-id',
    ...
    updateProfile: createMockEndpoint('user-updateProfile', async (data?: UpdateProfileRequest) => {
      return {
        id: 'mock-user-id',
        email: 'mock@example.com',
        firstName: data?.firstName || 'Mock',
  ...
  // Data section
  data = {
    saveDocument: createMockEndpoint('data-saveDocument', async (data?: SaveDocumentRequest) => {
      const now = new Date().toISOString();
      return {
        id: data?.id || `mock-doc-${Math.random().toString(36).substring(2, 9)}`,
    ...
    getDocument: createMockEndpoint('data-getDocument', async (data?: GetDocumentRequest) => {
      const now = new Date().toISOString();
      return {
    ...
    listDocuments: createMockEndpoint('data-listDocuments', async () => {
      ...
      const mockDocs: DocumentResponse[] = Array.from({ length: 5 }, (_, i) => ({
        id: `mock-doc-${i}`,
        data: { title: `Mock Document ${i}`, content: `This is mock document ${i}` },
        createdAt: now,
        updatedAt: now,
      }));
